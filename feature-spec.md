...
### 功能描述
通过 SSH `tail -f` 实时追踪 SLURM job 的 `.out` 文件内容，以只读终端风格展示。

### 触发方式
Job 看板（M2）中点击 `[查看日志 ↓]` 按钮，在对应 job 卡片下方内联展开。

### 日志文件路径推断
SLURM 默认输出路径为 `~/slurm-<JobID>.out`。
若该文件不存在，App 提示：`日志文件未找到，请确认 #SBATCH --output 路径`，并提供一个文本框供用户手动输入路径。
